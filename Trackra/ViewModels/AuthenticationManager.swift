...
final class AuthenticationManager: ObservableObject {
    ...
    func login(email: String, password: String, rememberMe: Bool = true) async {
        ...
        do {
            ...
            // Handle account not found - send access request
            if case .accountNotFound(let requestEmail) = error {
                // Send access request email
                Task {
                    await EmailService.shared.sendAccessRequestSilently(email: requestEmail)
                }
                errorMessage =
                    "Account not found. An access request has been sent to the administrator."
            } else {
