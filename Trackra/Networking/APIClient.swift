...
protocol APIClientProtocol {
    ...
    func createApplication(
        apiKey: String, role: String, company: String, appliedAt: Date, source: String,
        salaryRange: String, location: String, url: String
    ) async throws -> String
    ...
    func createActivity(
        apiKey: String, applicationId: String, type: ActivityType, occurredAt: Date, note: String
    ) async throws -> String
...
final class APIClient: APIClientProtocol {
    private let baseURL =
        "https://script.google.com/macros/s/AKfycbwMp-NQ3-SlnrdIQMboGLKjxX_YmLdOTL9dC2fUy65ekFBtnfwpslP1IIYE5u8VBUrB/exec"
        ...
        do {
            ...
            if let error = loginResponse.error {
                ...
                if error.lowercased().contains("not found")
                    || error.lowercased().contains("no account")
                    || error.lowercased().contains("user not found")
                    || error.lowercased().contains("invalid credentials")
                {
                    throw APIError.accountNotFound(email: email)
                }
